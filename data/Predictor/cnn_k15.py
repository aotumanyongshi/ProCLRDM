...
from data.utils.utils_predictor import EarlyStopping, seq2onehot, open_fa, open_exp
...
class CNN_K15_language:
    ...
    def train_with_loaders(self, train_loader, val_loader, output_dir):
        """
        使用已有的 DataLoader 进行模型训练

        Args:
            train_loader: 训练集 DataLoader
            val_loader: 验证集 DataLoader
            output_dir: 模型和日志保存目录
        """
        # 创建保存目录
        save_path = os.path.join(output_dir, self.model_name)
        os.makedirs(save_path, exist_ok=True)

        # 早停机制初始化（基于皮尔逊相关系数最大化）
        early_stopping = EarlyStopping(
            patience=self.patience,
            verbose=True,
            path=os.path.join(save_path, 'checkpoint.pth'),
            stop_order='max'  # 因为要最大化相关系数，所以用'max'
        )

        # 日志和模型保存路径
        train_log_filename = os.path.join(save_path, "train_log.txt")
        train_model_filename = os.path.join(save_path, "checkpoint.pth")
        print(f"训练结果将保存至: {save_path}")
        ...
        # 开始训练
        for epoch in tqdm(range(self.epoch), desc="训练进度"):
            # 训练阶段
            model.train()
            train_epoch_loss = []
            ...
            for batch_idx, (feature, label) in enumerate(train_loader):
                # 数据格式转换并移动到设备
                feature = feature.to(torch.float32).to(device).permute(0, 2, 1)  # 调整维度为 [batch, channel, length]
                label = label.to(torch.float32).to(device)

                # 前向传播
                outputs = model(feature)
                loss = criterion(label, outputs.flatten())
                ...
                # 打印训练日志（按设定的log_steps）
                if (batch_idx + 1) % self.log_steps == 0:
                    avg_loss = np.average(train_epoch_loss[-self.log_steps:])
                    print(
                        f"Epoch [{epoch + 1}/{self.epoch}], Step [{batch_idx + 1}/{len(train_loader)}], Batch Loss: {avg_loss:.4f}")
            ...
            with torch.no_grad():  # 关闭梯度计算，节省内存
                for feature, label in val_loader:
                    ...
                    outputs = model(feature)
                    valid_exp_real.extend(label.cpu().numpy().tolist())  # 转移到CPU并转为列表
                    valid_exp_pred.extend(outputs.flatten().cpu().numpy().tolist())

            # 计算验证集相关系数
            coefs = np.corrcoef(valid_exp_real, valid_exp_pred)[0, 1]
            pearson = pearsonr(valid_exp_real, valid_exp_pred)
            print(f"\nEpoch [{epoch + 1}/{self.epoch}] 验证结果:")
            print(f"平均训练损失: {np.average(train_epoch_loss):.4f}")
            print(f"验证集相关系数: {coefs:.4f}")
            print(f"皮尔逊相关系数 (含p值): {pearson}\n")

            # 写入训练日志
            if (epoch + 1) % self.log_steps == 0:
                with open(train_log_filename, "a") as f:
                    f.write(f"epoch={epoch + 1}, train_loss={np.average(train_epoch_loss):.4f}, val_corr={coefs:.4f}\n")

            # 保存模型（按设定的save_steps）
            if (epoch + 1) % self.save_steps == 0:
                torch.save(model.state_dict(), train_model_filename)
                print(f"模型已保存至 {train_model_filename}")

            # 早停检查
            early_stopping(val_loss=coefs, model=self.model)
    ...
    def predict(self, model_path, data_path):
        ...
        path_pred_save = '{}/nature_preds.txt'.format(model_path)
