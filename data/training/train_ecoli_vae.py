...
from data.models.ecoli_vae_model import VAE # 导入 PyTorch 版本的 VAE 类
...
# 大肠杆菌长度为 165 bp
# 这里酿酒酵母长度为80bp
dataset_path = '/root/autodl-tmp/sjy/dachang1/data/raw_sequences/ecoli_data/seq10000.txt'
...
# 7. 训练完成后，保存模型
save_path = '/root/autodl-tmp/sjy/dachang1/data/training/ecilo_vae/my_vae_model_state_dict.pth'  # 根据你的目录结构调整保存路径
...
# TODO: 如果你还需要计算其他指标 (例如碱基准确率)，可以在这里添加计算和打印的逻辑
# Example Base Accuracy Calculation:
# This requires iterating through original and reconstructed tensors *before* converting to strings
# and comparing base by base. You could add this inside the evaluation loop.


print("\n--- VAE 重构能力评估结束 ---")

# 8. 使用训练好的模型生成新的合成序列 (这部分通常在单独的生成脚本中进行，这里保留作为示例)
# model.eval() # 确保模型仍处于评估模式
# with torch.no_grad(): # 在 no_grad 模式下进行推理
#      num_sequences_to_generate = 10 # Example: generate 10 sequences
#      # 使用 sample 方法生成潜在向量 z_0
#      generated_latent_vectors = model.sample(num_sequences_to_generate) # Shape (num_sequences_to_generate, latent_dim)
#      print(f"\nGenerated {num_sequences_to_generate} latent vectors from VAE sample method.")
#
#      # 解码潜在向量以生成序列 (获取概率输出)
#      generated_output_probs = model.decode(generated_latent_vectors, apply_sigmoid=True) # Shape (num_sequences_to_generate, 1, 4, 185)
#      print(f"Decoded latent vectors to probability tensor with shape: {generated_output_probs.shape}")
#
#      # 将输出张量转换为序列 (使用你修改后的 decoder2seq_torch 函数或其他转换函数)
#      # This part requires your specific conversion logic from probability tensor or index tensor to sequence string
#      # Example:
#      # decoded_generated_sequences = [reconstructed_to_sequence(generated_output_probs[i].squeeze(0), SEQ_LENGTH_RAW, PADDING_RAW) for i in range(num_sequences_to_generate)]
#
#      # TODO: Implement the actual conversion from probability tensor to sequence string here (can reuse reconstructed_to_sequence)
#      # Placeholder:
#      decoded_generated_sequences_str = ["GENERATED_SEQ"] * num_sequences_to_generate # Replace with actual conversion
#      print("\nExample of generated sequences:")
#      for i, seq in enumerate(decoded_generated_sequences_str[:min(5, num_sequences_to_generate)]): # Print first 5 example sequences
#           print(f"Seq {i+1}: {seq}") # Corrected f-string index
#
#      # TODO: 将生成的序列保存到文件 (例如 Nat.txt)
#      # save_generated_sequences_to_file(decoded_generated_sequences_str, 'generated_vae_sequences.txt')