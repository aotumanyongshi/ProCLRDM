...
# 导入自定义的模型和工具函数
from data.models.ecoli_vae_model import VAE
from data.models.diffusion_model import LatentDiffusionModel, TransformerDenoiseModel
...
# VAE 参数
VAE_LATENT_DIM = 256
VAE_MODEL_PATH = '/root/autodl-tmp/sjy/dachang1/data/training/ecilo_vae/my_vae_model_state_dict.pth'

# 数据路径
DATA_PATH = '/root/autodl-tmp/sjy/dachang1/data/raw_sequences/ecoli_data/ecoli_data.txt'
EXPRESSION_DATA_PATH = '/root/autodl-tmp/sjy/dachang/data/raw_sequences/ecoli_data/exp.txt'

# 序列参数
SEQ_LENGTH_RAW = 165  # 序列长度为165bp
PADDING_RAW = 10
...
KMER_PLOT_DIR = os.path.join(CHECKPOINT_DIR, 'kmer_plots')  # K-mer可视化目录
os.makedirs(KMER_PLOT_DIR, exist_ok=True)
MIDDLE_CHECKPOINT_DIR = os.path.join(CHECKPOINT_DIR, 'middle_checkpoints')  # 中间检查点目录
os.makedirs(MIDDLE_CHECKPOINT_DIR, exist_ok=True)  # 创建中间检查点专用目录

# 稳健K-mer评估参数
USE_ROBUST_KMER_EVAL = True
K_FOR_ROBUST_EVALUATION = 6
NUM_SAMPLES_FOR_ROBUST_EVAL = 10000
ROBUST_EVAL_FREQUENCY_EPOCHS = 50  # 评估频率，每50轮一次
...
print(f"每 {ROBUST_EVAL_FREQUENCY_EPOCHS} 轮强制保存一次中间检查点")
        ...
        if generated_sequences_for_eval and validation_original_sequences_for_robust_eval:
            ...
            if kmer_correlation > best_kmer_correlation_overall:
                ...
                try:
                    ...
                    print(f"  警告: 保存最佳检查点失败: {e}")
            else:
                epochs_since_last_improvement += 1

            # 4. 强制保存中间检查点（新增逻辑）
            middle_checkpoint_name = f'middle_model_epoch_{current_epoch}.pth'
            middle_checkpoint_path = os.path.join(MIDDLE_CHECKPOINT_DIR, middle_checkpoint_name)
            try:
                middle_checkpoint_data = {
                    'model_state_dict': diffusion_model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'scheduler_state_dict': scheduler.state_dict() if scheduler is not None else None,
                    'epoch': current_epoch,
                    'current_kmer_correlation_val': kmer_correlation
                }
                torch.save(middle_checkpoint_data, middle_checkpoint_path)
                print(f"  中间检查点已保存至: {middle_checkpoint_path}")
            except Exception as e:
                print(f"  警告: 保存中间检查点失败: {e}")
...
print(f"中间检查点保存于: {MIDDLE_CHECKPOINT_DIR}")
